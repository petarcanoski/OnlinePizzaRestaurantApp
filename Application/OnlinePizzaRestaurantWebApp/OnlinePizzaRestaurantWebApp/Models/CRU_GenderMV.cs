...
namespace OnlinePizzaRestaurantWebApp.Models
{
    public class CRU_GenderMV
    {
        ...
        {
            GetAllGenders();
            var editgender = new OnlinePizzaRestaurantDbEntities().Genders.Where(g => g.GenderID == id).FirstOrDefault();
            if (editgender != null)
            {
        ...
        private void GetAllGenders()
        {
            ...
            foreach (var gender in new OnlinePizzaRestaurantDbEntities().Genders.ToList())
            {
