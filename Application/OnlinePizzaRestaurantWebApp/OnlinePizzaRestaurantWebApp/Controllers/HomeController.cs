...
namespace OnlinePizzaRestaurantWebApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult SubscribeEmail(string subs_email)
        {
            var check = DB.SubscribeEmailTables.Where(x => x.SubscribeEmailAddress.Trim().ToUpper() == subs_email.Trim().ToUpper()).FirstOrDefault();
            if (check == null)
            {
                var email = new SubscribeEmailTable
                {
                    SubscribeEmailAddress = subs_email,
        ...
        public ActionResult ProductDetail(int? itemid)
        {
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(Session["UserID"])))
            {
                ...
                var review_existing = DB.StockItemReviewTables.Where(u => u.Review_UserID == userid && u.StockItemID == itemid).FirstOrDefault();
            ...
            for (int i = 0; i < reviews.Count(); i++)
            {
                ...
                var review_user = DB.UserTables.Find(reviews[i].Review_UserID).UserName;
                var userdetail = DB.UserTables.Find(reviews[i].Review_UserID).UserDetailTable;
            ...
            foreach (var related_item in DB.StockItemTables.Where(c => c.StockItemCategoryID == item.StockItemCategoryID).ToList())
            {
                ...
                item_detail.Related_Items.Add(new ItemMV()
                {
                    ...
                    OrderType = related_item.OrderTypeTable.OrderTpe,
        ...
        public ActionResult ProductDetail(ItemDetailMV itemDetailMV)
        {
            ...
            int userid = 0;
            int.TryParse(Convert.ToString(Session["UserID"]), out userid);
            var review_existing = DB.StockItemReviewTables.Where(u => u.Review_UserID == userid && u.StockItemID == itemDetailMV.StockItemID).FirstOrDefault();
            if (review_existing != null)
            {
            ...
            else
            {
                ...
                itemreview.Review_UserID = userid;
